...
public class SolarChunFen {
     ...
     *
     * @param year
     * @param data 月份占两位，日占两位，如一月一日为：0101，五月十日为0510
     * @return
    ...
    public static String getSolatName(Date date) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        String dateStr = df.format(date);
        int year = Integer.parseInt(dateStr.substring(0, 4));
        int month = Integer.parseInt(dateStr.substring(5, 7));
        int day = Integer.parseInt(dateStr.substring(8, 10));
        String dateStr2 = dateStr.substring(5, 10);

        if (year != mYear) {
            solarTermToString(year);
        }
        if (mSolarData.contains(dateStr2)) {
            return mSolarName.get(mSolarData.indexOf(dateStr2));
        } else {
            while (true) {
                dateStr2 = String.format("%02d", month) + "-" + String.format("%02d", day);
                if (mSolarData.contains(dateStr2)) {
                    break;
                }
                if (day == 1 && month != 3) {
